...
/// Renders a faithful PDF from the canonical clinical document.
///
/// Follows ADR-0006 & BUILD-PLAN Milestone 11:
/// - Canonical document coordinate system ensures zero displacement.
/// - Layer 0: Template background (edge-to-edge custom template or default layout).
/// - Layer 1: Vector digital ink layer directly from canonical InkDocument.
class PdfExporter {
  /// Converts a document coordinate (in editor pixels at 96 DPI: 1mm = 3.78px)
  ...
  /// Generates a PDF byte buffer from a [ClinicalDocument].
  static Future<Uint8List> generatePdfBytes({
    ...
    final widthMm = document.page.width;
    final heightMm = document.page.height;
    ...
    final strokes = document.ink.strokes;
    final doctor = document.doctorSnapshot;
    final patient = document.patientSnapshot;
    final effectiveDate = dateString ?? DateTime.now().toIso8601String().split('T').first;
        ...
        build: (pw.Context context) {
          return pw.Stack(
            children: [
              ...
              if (document.templateBytes != null)
                pw.Positioned(
                  left: 0,
                  top: 0,
                  right: 0,
                  bottom: 0,
                  child: pw.Image(
                    pw.MemoryImage(document.templateBytes!),
                    fit: pw.BoxFit.fill,
                  ),
                )
              else
  ...
  /// Generates a PDF file from a [ClinicalDocument] and writes to [targetFile].
